...
func (m *MergeController) RevertBranch(projectPath, branchName string) error {
	...
	out, err := runGitOutput(projectPath, "log", "--oneline", "-1")
	if err != nil {
		return fmt.Errorf("get last commit: %w", err)
	}
	if !strings.Contains(out, "swarm: merge "+branchName) {
		return fmt.Errorf("last commit is not the merge of %s", branchName)
	}
	return runGit(projectPath, "revert", "--no-edit", "-m", "1", "HEAD")
}
