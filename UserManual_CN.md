...
欢迎使用 **CodeClaw（代码龙虾）**！本工具专为简化多种 AI 编程命令行工具（Claude Code, Codex, Gemini CLI）的配置、多项目管理与启动流程而设计。

以下是详细的操作指南：

## 1. 启动与环境检测
首次运行程序时，CodeClaw（代码龙虾） 会自动检查您的系统环境：
...
## 2. 侧边栏导航 (Sidebar)
CodeClaw（代码龙虾）采用垂直侧边栏设计，您可以轻松切换不同的 AI 工具：
...
*   **自动清理**：在该模式下启动工具时，CodeClaw（代码龙虾） 会自动清除本工具产生的自定义代理配置、环境变量以及官方工具的配置文件（如 Claude 的 `~/.claude` 目录），确保环境纯净。
