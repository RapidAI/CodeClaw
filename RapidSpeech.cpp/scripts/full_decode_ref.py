...
def rope_single(x, pos, rotary_dim, theta):
    """Apply RoPE to x[1, n_heads, 1, head_dim] at position pos."""
    dp = rotary_dim // 2
...
def rope_seq(x, rotary_dim, theta):
    """Apply RoPE to x[1, n_heads, seq, head_dim]."""
...
for i in range(6):
    res = x
    x = rms_norm(x, weights[f"model.encoder.layers.{i}.input_layernorm.weight"])
    ...
    res = x
    x = rms_norm(x, weights[f"model.encoder.layers.{i}.post_attention_layernorm.weight"])
...
enc = rms_norm(x, weights["model.encoder.layer_norm.weight"])
...
for step in range(100):
    ...
    for i in range(n_dec):
        # Self-attention
        res = x
        x = rms_norm(x, weights[f"model.decoder.layers.{i}.input_layernorm.weight"])
        ...
        # Cross-attention
        res = x
        x = rms_norm(x, weights[f"model.decoder.layers.{i}.post_attention_layernorm.weight"])
        ...
        # SwiGLU FFN
        res = x
        x = rms_norm(x, weights[f"model.decoder.layers.{i}.final_layernorm.weight"])
    ...
    logits = F.linear(rms_norm(x, dec_norm), embed)
