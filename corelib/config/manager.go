...
// Manager provides structured read/write access to AppConfig.
type Manager struct {
	store  ConfigStore
...
func (m *Manager) GetConfig(section string, unmasked ...bool) (string, error) {
...
func (m *Manager) configOverview(cfg corelib.AppConfig) string {
...
func (m *Manager) formatToolConfig(name string, tc corelib.ToolConfig, raw bool) string {
	var b strings.Builder
...
func (m *Manager) formatRemoteConfig(cfg corelib.AppConfig, raw bool) string {
	var b strings.Builder
	b.WriteString("=== 远程设置 ===\n")
...
func (m *Manager) formatProjectsConfig(cfg corelib.AppConfig) string {
	var b strings.Builder
	b.WriteString("=== 项目列表 ===\n")
...
func (m *Manager) formatMaclawLLMConfig(cfg corelib.AppConfig, raw bool) string {
	var b strings.Builder
...
func (m *Manager) formatMaclawRoleConfig(cfg corelib.AppConfig) string {
	var b strings.Builder
...
func (m *Manager) formatPowerConfig(cfg corelib.AppConfig) string {
	var b strings.Builder
	b.WriteString("=== 电源管理 ===\n")
	b.WriteString(fmt.Sprintf("power_optimization: %v\n", cfg.PowerOptimization))
	b.WriteString(fmt.Sprintf("screen_dim_timeout_min: %d\n", cfg.ScreenDimTimeoutMin))
...
func (m *Manager) UpdateConfig(section, key, value string) (string, error) {
	cfg, err := m.store.LoadConfig()
	if err != nil {
		return "", fmt.Errorf("failed to load config: %w", err)
	}
...
// machineSpecificKeys lists keys that must not be overwritten during import.
...
// it into the current config.
// ---------------------------------------------------------------------------

func (m *Manager) ImportConfig(jsonData string) (*ImportReport, error) {
	...
	if err := m.store.SaveConfig(mergedCfg); err != nil {
...
// mergeImport recursively merges importMap values into currentMap.
...
func (m *Manager) GetSchema() []ConfigSection {
	return m.schema
}

// SchemaJSON serialises the schema to JSON.
func (m *Manager) SchemaJSON() (string, error) {
...
func (m *Manager) validateChange(section, key, value string) error {
...
func (m *Manager) applyChange(cfg *corelib.AppConfig, section, key, value string) (string, error) {
	sec := strings.ToLower(strings.TrimSpace(section))
	k := strings.ToLower(strings.TrimSpace(key))
...
func (m *Manager) applyProjectsChange(cfg *corelib.AppConfig, key, value string) (string, error) {
	switch key {
...
func (m *Manager) applyRemoteChange(cfg *corelib.AppConfig, key, value string) (string, error) {
	switch key {
	case "remote_enabled":
...
func (m *Manager) applyMaclawLLMChange(cfg *corelib.AppConfig, key, value string) (string, error) {
	switch key {
	case "maclaw_llm_url":
...
func (m *Manager) applyMaclawRoleChange(cfg *corelib.AppConfig, key, value string) (string, error) {
	switch key {
...
func (m *Manager) applyGeneralChange(cfg *corelib.AppConfig, key, value string) (string, error) {
	switch key {
	case "active_tool":
		old := cfg.ActiveTool
...
func (m *Manager) applyPowerChange(cfg *corelib.AppConfig, key, value string) (string, error) {
	switch key {
	case "power_optimization":
		old := fmt.Sprintf("%v", cfg.PowerOptimization)
		cfg.PowerOptimization = strings.EqualFold(value, "true")
