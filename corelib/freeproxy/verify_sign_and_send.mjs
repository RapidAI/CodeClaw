// Sign with WASM in Node.js and send the request to verify signing works
// Usage: node verify_sign_and_send.mjs <cookie> <conversationId>
...
if (!cookie || !convId) {
    console.error('Usage: node verify_sign_and_send.mjs <cookie> <conversationId>');
    process.exit(1);
}
...
const sr = getSign(body, url);
console.log('Sign result:', JSON.stringify(sr));
