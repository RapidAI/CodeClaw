// Test multiple sign calls to see if sign changes with nonce
...
// Also verify: does v1Sign(ts, body, nonce) match the WASM output?
import { createHash } from 'crypto';
function v1Sign(ts, body, nonce) {
    return createHash('md5').update(`${ts}${body}${nonce}`).digest('hex').toUpperCase();
}

const body = '{"stream":true,"botCode":"AI_SEARCH","conversationId":"12345","question":"hi","agentId":""}';
console.log('=== Multiple sign calls ===');
for (let i = 0; i < 5; i++) {
    ...
    // Check if it matches v1 formula: MD5(timestamp + body + nonce)
    const v1 = v1Sign(ts, body, nonce);
    const match = sign === v1 ? 'MATCH' : 'DIFFER';
    console.log(`[${i}] sign=${sign} nonce=${nonce} ts=${ts} v1=${v1} ${match}`);
}
