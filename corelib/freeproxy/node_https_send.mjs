// Send signed v2/chat request using Node.js native https module
// Usage: node node_https_send.mjs <cookie> <conversationId>
...
if (!cookie || !convId) {
    console.error('Usage: node node_https_send.mjs <cookie> <conversationId>');
    process.exit(1);
}

// === WASM setup (browser-simulated) ===
let wasm, V = 0;
...
console.log('token from cookie:', tokenValue);
...
const req = https.request(options, (res) => {
    ...
    res.on('data', (chunk) => {
        data += chunk;
        if (data.length < 2000) process.stdout.write(chunk);
    });
    res.on('end', () => {
        console.log('\n\n=== Response complete ===');
