...
func NewContextBridge() *ContextBridge {
	return &ContextBridge{contexts: make(map[string]*ProjectContext)}
}
...
func (b *ContextBridge) ExtractFromEvents(projectPath string, events []remote.ImportantEvent) {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	for _, e := range events {
		...
		case "file.change", "file.create", "file.delete":
			...
			ctx.FileChanges = append(ctx.FileChanges, FileChangeRecord{
				File: e.Summary, Action: action, Timestamp: ts, SessionID: e.SessionID,
			})
		case "command.execute":
			ts := time.Unix(e.CreatedAt, 0)
			if isSignificantCommand(e.Summary) {
				ctx.Decisions = append(ctx.Decisions, DecisionRecord{
					Description: e.Summary, Timestamp: ts, SessionID: e.SessionID,
				})
			}
		}
	}
