...
// NewTaskSplitter creates a TaskSplitter with the given LLM caller.
func NewTaskSplitter(caller SwarmLLMCaller) *TaskSplitter {
	return &TaskSplitter{caller: caller}
}
...
func (s *TaskSplitter) SplitViaAgent(agentOutput string) ([]SubTask, error) {
	...
	if s.caller == nil {
		return nil, fmt.Errorf("LLM caller not configured, cannot parse unstructured agent output")
	}
