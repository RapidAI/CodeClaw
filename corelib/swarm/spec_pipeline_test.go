...
func TestFormatCriteria_Pipeline(t *testing.T) {
	tests := []struct {
		name     string
		criteria []string
		want     string
...
func TestSubTask_AcceptanceCriteria_Pipeline(t *testing.T) {
	task := SubTask{
		Index:              0,
...
func TestTaskVerifier_WithAcceptanceCriteria_Pipeline(t *testing.T) {
...
func TestSplitViaAgent_WithAcceptanceCriteria_Pipeline(t *testing.T) {
...
FAIL`
	...
	if len(failures) > 0 && failures[0].TestName != "TestLogin" {
		t.Errorf("TestName = %q, want TestLogin", failures[0].TestName)
	}
}

func TestParseTestFailures_Empty(t *testing.T) {
	failures := ParseTestFailures("")
	if failures != nil {
		t.Errorf("expected nil for empty output, got %v", failures)
	}
}
...
func TestInferCompileCommand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.techStack, func(t *testing.T) {
			got := InferCompileCommand(tt.techStack)
			if got != tt.want {
				t.Errorf("InferCompileCommand(%q) = %q, want %q", tt.techStack, got, tt.want)
			}
		})
	}
}
...
func TestIsActiveSessionStatus(t *testing.T) {
	if !isActiveSessionStatus(SessionStarting) {
		t.Error("SessionStarting should be active")
	}
	if !isActiveSessionStatus(SessionRunning) {
		t.Error("SessionRunning should be active")
	}
	if isActiveSessionStatus(SessionExited) {
		t.Error("SessionExited should not be active")
	}
	if isActiveSessionStatus(SessionError) {
		t.Error("SessionError should not be active")
	}
}
