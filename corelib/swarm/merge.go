...
func (m *MergeController) RevertBranch(projectPath, branchName string) error {
	...
	out, err := swarmRunGitOutput(projectPath, "cat-file", "-p", "HEAD")
	if err != nil {
	...
	return swarmRunGit(projectPath, "reset", "--hard", "HEAD~1")
}
...
func (m *MergeController) runCompile(projectPath, compileCmd string) error {
	...
	return swarmRunShellCommand(projectPath, parts[0], parts[1:]...)
}

// swarmRunShellCommand executes a command in the given directory.
// Unlike the GUI version, it does not call hideCommandWindow (GUI-specific
// Windows window hiding is not needed in corelib).
func swarmRunShellCommand(dir, name string, args ...string) error {
