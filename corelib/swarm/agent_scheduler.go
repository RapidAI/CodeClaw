...
// createAgent creates a single SwarmAgent backed by a session via
// SwarmSessionManager. It renders the role-specific system prompt, creates a
// session via sessionMgr.Create with ProjectPath pointing to the worktree,
...
func (o *SwarmOrchestrator) createAgent(
	...
	o.addTimelineEvent(run, "agent_created",
		fmt.Sprintf("Created %s agent (task %d) → session %s", role, taskIndex, session.SessionID()),
		agent.ID)

	return agent, nil
}

// waitForAgent polls the session status until the agent completes,
...
func (o *SwarmOrchestrator) waitForAgent(run *SwarmRun, agent *SwarmAgent, timeout time.Duration) error {
	if o.sessionMgr == nil || agent.SessionID == "" {
	...
	for {
		...
		case <-deadline:
			...
			_ = o.sessionMgr.Kill(agent.SessionID)
			now := time.Now()
			agent.Status = "failed"
		...
		case <-ticker.C:
			session, ok := o.sessionMgr.Get(agent.SessionID)
			if !ok {
				now := time.Now()
				agent.Status = "failed"
			...
			case SessionExited:
				...
				agent.Status = "completed"
				agent.CompletedAt = &now
				agent.Output = session.SessionSummary().ProgressSummary
				return nil
			...
			case SessionError:
				...
				agent.Status = "failed"
				agent.CompletedAt = &now
				agent.Error = session.SessionSummary().LastResult
...
// retries up to MaxAgentRetries times on error. Uses tool.Selector to pick
