...
type FeedbackLoop struct {
	caller    SwarmLLMCaller
	maxRounds int
	round     int
	history   []SwarmRound
}

// NewFeedbackLoop creates a FeedbackLoop with the given LLM caller and max rounds.
// caller may be nil; ClassifyFailures will return a descriptive error in that case.
...
func (f *FeedbackLoop) ClassifyFailures(failures []TestFailure) ([]ClassifiedFailure, error) {
	...
	if f.caller == nil {
		return nil, fmt.Errorf("LLM caller not configured, cannot classify failures")
	}

	prompt := buildClassificationPrompt(failures)
	body, err := f.caller.CallLLM(prompt, 0.1, 60*time.Second)
	if err != nil {
	...
	for _, tf := range failures {
		failMap[tf.TestName] = tf
	}
