...
func TestSplitRequirements_NilCaller(t *testing.T) {
	ts := NewTaskSplitter(nil)
	_, err := ts.SplitRequirements("some requirements", "Go")
	if err == nil {
		t.Fatal("expected error when caller is nil")
	}
	if !strings.Contains(err.Error(), "LLM caller not configured") {
