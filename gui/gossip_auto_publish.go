...
type AutoPublishTrigger struct {
	...
	enabled       func() bool
	gossipAllowed func() bool // Hub security policy check (Req 6.3)
	buffer        []ChatExchange
...
func NewAutoPublishTrigger(client *GossipClient, enabledFn func() bool) *AutoPublishTrigger {
	return &AutoPublishTrigger{
		client:        client,
		enabled:       enabledFn,
		gossipAllowed: func() bool { return true }, // default: allowed
		httpClient:    &http.Client{Timeout: 30 * time.Second},
	}
}

// SetGossipAllowedFn sets the Hub security policy check function (Req 6.3).
// When this returns false, auto-publish is skipped regardless of the local enabled setting.
func (t *AutoPublishTrigger) SetGossipAllowedFn(fn func() bool) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.gossipAllowed = fn
}
...
func (t *AutoPublishTrigger) OnChatCompleted(userMsg, assistantReply string) {
	...
	// Hub security policy: skip when gossip is disallowed (Req 6.3)
	if t.gossipAllowed != nil && !t.gossipAllowed() {
		t.mu.Unlock()
		log.Printf("[gossip-auto] gossip disabled by hub security policy, skipping")
		return
	}
