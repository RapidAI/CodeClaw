...
// ---------------------------------------------------------------------------
// BackgroundLoopManager — manages background agent loop lifecycle with
...
type BackgroundLoopManager struct {
	...
	// Callback when loop state changes (for Wails event emission)
	OnChange func()
}
...
// SpawnOrQueue creates a loop if slot available, otherwise queues and returns
// a channel that will receive the LoopContext when the slot opens.
// Spawn creates a new Background Loop if the slot is available.
// Returns nil if the slot is full (caller should use SpawnOrQueue for
// automatic queuing).
...
// spawnLocked creates a loop context and registers it. Caller must hold m.mu.
...
func (m *BackgroundLoopManager) Stop(loopID string) {
	...
	// Dequeue next pending task for this slot
	...
	if next != nil {
		...
		// spawnLocked already called notifyChange
		return
	}
