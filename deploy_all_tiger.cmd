...
REM ============================================================
REM  deploy_all_tiger.cmd - TigerClaw (oem_qianxin) brand deploy
REM  Same logic as deploy_all.cmd but builds with -tags oem_qianxin
REM  and uses tigerclaw-hub / tigerclaw-hubcenter binary names.
REM  start.sh is auto-patched via sed on the remote host.
REM ============================================================
...
echo.
echo [TigerClaw Deploy] Brand: oem_qianxin
...
echo [5/5] Running remote build and deployment (TigerClaw)...
...
(
  ...
  echo.
  echo echo "[remote] Building hub (TigerClaw)..."
  echo GOPROXY="$GOPROXY" CGO_ENABLED="$CGO_ENABLED" go build -tags oem_qianxin -o "$BUILD_ROOT/tigerclaw-hub" ./hub/cmd/hub
  echo echo "[remote] Building hubcenter (TigerClaw)..."
  echo GOPROXY="$GOPROXY" CGO_ENABLED="$CGO_ENABLED" go build -tags oem_qianxin -o "$BUILD_ROOT/tigerclaw-hubcenter" ./hubcenter/cmd/hubcenter
  echo.
  echo deploy_one^(^) {
  echo   source_dir="$1"
  echo   target_dir="$2"
  ...
  echo     # Rebrand: replace maclaw-hubcenter before maclaw-hub to avoid partial match
  echo     sed -i 's/maclaw-hubcenter/tigerclaw-hubcenter/g' "$target_dir/start.sh"
  echo     sed -i 's/maclaw-hub/tigerclaw-hub/g' "$target_dir/start.sh"
  ...
  echo }
  echo.
  echo echo "[remote] Deploying hub files (TigerClaw)..."
  echo deploy_one "$SRC_ROOT/hub" "$REMOTE_HUB_DIR" "$BUILD_ROOT/tigerclaw-hub" "tigerclaw-hub"
  echo echo "[remote] Deploying hubcenter files (TigerClaw)..."
  echo deploy_one "$SRC_ROOT/hubcenter" "$REMOTE_HUBCENTER_DIR" "$BUILD_ROOT/tigerclaw-hubcenter" "tigerclaw-hubcenter"
  echo.
  ...
  echo fi
  echo.
  echo echo "[remote] Restarting hub (TigerClaw)..."
  echo if [ -x "$REMOTE_HUB_DIR/start.sh" ]; then
  echo   cd "$REMOTE_HUB_DIR"
  echo   ./start.sh
  echo fi
  echo echo "[remote] Restarting hubcenter (TigerClaw)..."
  ...
  echo echo "Remote build and deploy finished (TigerClaw)."
) > "%REMOTE_SCRIPT%"
