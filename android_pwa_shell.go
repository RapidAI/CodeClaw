...
android {
    ...
    applicationVariants.configureEach { variant ->
        variant.outputs.configureEach {
            outputFileName = "codeclaw-${variant.buildType.name}.apk"
        }
    }
}
...
func buildAndroidSigningExampleCMD() string {
	return `@echo off
set "RELEASE_STORE_FILE=C:\path\to\codeclaw-release.jks"
set "RELEASE_STORE_PASSWORD=change-me"
set "RELEASE_KEY_ALIAS=codeclaw"
...
func buildIOSShellReadme(appName string, bundleID string, hubCenterURL string, startURL string, projectName string) string {
	return fmt.Sprintf(
		"# CodeClaw iOS Shell\n\n"+
			"This iOS shell project was generated by CodeClaw's Go-based mobile shell generator.\n\n"+
			"## Defaults\n\n"+
			"- App name: CodeClaw\n"+
			"- Bundle ID: %s\n"+
			"- Start URL: %s\n"+
			...
			"Current filenames:\n\n"+
			"- Debug simulator app: `codeclaw-ios-simulator.app`\n"+
			"- Release simulator app: `codeclaw-ios-simulator-release.app`\n"+
...
func buildMobileDistReadme() string {
	return "# Mobile Deliverables\n\n" +
		...
		"- `../android/codeclaw-release.jks`\n" +
		"- `../android/signing.env.cmd`\n\n" +
		"## iOS\n\n" +
		"- `codeclaw-ios-simulator.app`\n" +
		"  Debug iOS simulator app bundle, generated on macOS.\n" +
		"- `codeclaw-ios-simulator-release.app`\n" +
...
COPIED_APP="$DIST_DIR/codeclaw-ios-simulator.app"
