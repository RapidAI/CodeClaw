...
REM Clean DLL/import-lib artifacts that confuse the MinGW static linker
powershell -NoProfile -Command "Get-ChildItem '%~dp0RapidSpeech.cpp\build' -Recurse -Include '*.dll','*.dll.a' | Remove-Item -Force"
echo [INFO] Cleaned DLL artifacts from RapidSpeech build.
